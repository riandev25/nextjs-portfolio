...
const Navbar = ({ isOpen }) => {
  const [isOpen, drawerIsOpen] = useReducer((state) => {
    return !state;
  }, isOpen);
  ...
  return (
    <div
      className={`z-[60] ${
        !isOpen && 'hidden'
      } bg-red-400 absolute top-0 right-0 justify-between items-center w-[70%] h-screen flex md:relative md:w-auto md:order-1`}
      id='navbar-sticky'
    >
      <div className='h-screen relative flex justify-center items-center flex-auto bg-black'>
        <button
          type='button'
