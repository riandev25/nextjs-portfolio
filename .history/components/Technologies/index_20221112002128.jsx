  ...
  return (
    <div className='pt-4 flex flex-col w-full h-screen gap-11 justify-center items-center text-white px-20 bg-bg_about_image bg-fixed'>
      ...
      <div className='max-w-4xl flex flex-row'>
        ...
        <section className='relative flex flex-col bg-white drop-shadow-lg border-2 border-[#b2fcfb] w-full p-7 gap-8 rounded-lg image-filter'>
          <div className='absolute w-full h-full border-2 border-[#bf2cfb]'></div>
          <ul className='grid grid-cols-3 gap-3'>
            {FRONTEND.map((item) => {
              return (
                <li
                  key={item}
                  className='grid grid-cols-3 border-2 rounded-lg bg-gray-500'
                >
