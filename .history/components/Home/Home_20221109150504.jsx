...
import GearImage from '../../assets/images/gearwheel.png';
...
const Home = () => {
  const bgStyle = { backgroundImage: GearImage };
  return (
    <header
      className='flex justify-center items-center w-full h-screen px-20 bg-repeat'
      style={bgStyle}
    >
      <section className='w-6/12 space-y-4'>
        ...
        <ul className='flex flex-row gap-4'>
          {images.map((item) => {
            ...
            return (
              <li
                className='block p-3 border rounded-lg border-[#b2fcfb] transition-all hover:scale-110'
                key={item.id}
              >
